...
public:
  ...
  struct Node: public TDataContainerProvider::template Node<Node, Edge>, public std::enable_shared_from_this<Node> {
    ...
    inline Node(const TopoMap::Node* region, Args&&... args):
...
// a->b
  struct Edge: public TDataContainerProvider::template Edge<Node, Edge>, public std::enable_shared_from_this<Edge> {
    ...
    const TopoMap::Edge* topoEdge = nullptr;
    const TopoMap::Node* topoNode = nullptr;
    
    inline Edge(Node* a, Node* b, const TopoMap::Edge* edge): a(a), b(b), topoEdge(edge) {
      if(!a || !b)
    ...
    inline Edge(Node* a, Node* b, const TopoMap::Node* node): a(a), b(b), topoNode(node) {
      if(!a || !b)
        throw std::runtime_error("Created planning edge with a or b nullptr");
  ...
  template<typename... Args>
  inline Node *addLonelyNode(const TopoMap::Node *region, typename Node::State initialState, Args&&... args) {
  ...
  inline void resetApproachExitPosesMap(std::map<const TopoMap::Edge*, std::pair<Node*, Node*>> && map, unsigned long long topoVersion) {
    approachExitPosesMap = map;
    lastTopoMapVersion = topoVersion;
  }
  
  template<typename... Args>
  inline void setStartNode(const TopoMap::Node* region, Args&&... args) {
    if(startNode) {
    ...
    for(const TopoMap::Edge* edge: region->getOutEdges()) {
      addEdge(startNode, approachExitPosesMap.at(edge).first, region);
    }
  }
  
  template<typename... Args>
  inline void setEndNode(const TopoMap::Node* region, Args&&... args) {
    if(endNode) {
    ...
    for(const TopoMap::Edge* edge: region->getInEdges()) {
  ...
  const std::pair<Node*, Node*>& getApproachExitPose(const TopoMap::Edge* e) const {
...
protected:
  ...
  std::map<const TopoMap::Edge*, std::pair<Node*, Node*>> approachExitPosesMap;
