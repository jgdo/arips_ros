...
template <>
struct BaseTraits<StepEdgeModule>: public ApproachExit3DEdgeModuleBaseTraits {
	struct CostSettings {
		double time_costs = 1;
  ...
  struct MapStepData {
    std::map<std::string, StepInfo> steps;
		CostSettings defaultUpCosts, defaultDownCosts, defaultCrossOverCosts, defaultCrossGravelCosts;
  };
  ...
  struct EdgeStepData {
    std::string stepName;
    enum StepType {
      UP, DOWN, CROSS_OVER, CROSS_GRAVEL
    } type;
...
class StepEdgeModule:
        public ApproachExit3DEdgeModuleBase<StepEdgeModule, FlatGroundModule>,
...
public:
	...
	virtual std::pair<double, toponav_core::LocalPositionConstPtr> computeTransitionCost(const toponav_core::TopoMap::Edge* edge, const toponav_core::LocalPosition &approachPose, boost::any *pathData) override;
	...
	static void createNewMapStep(toponav_core::TopoMap *map, std::string name, tf2::Stamped<tf2::Vector3> const &start,
                               const tf2::Stamped<tf2::Vector3> &end);
	
	static void initEdgeData(toponav_core::TopoMap::Edge *edge, std::string stepName, EdgeStepData::StepType stepType);
...
private:
	enum {
		IDLE, CREATING_EDGE_APPROACH, CREATING_EDGE_EXIT, CREATING_MAP_STEP, WAITING_STEP_SELECTION
	} vizState = IDLE;
	...
	void setEdgeUpDown(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback, const toponav_core::TopoMap::Edge *data);
  
  void setApproachFromStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback,
                             const toponav_core::TopoMap::Edge *data);
  
  void visualizeMapStep(StepInfo* stepInfo);
  
  void processMapStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback, StepInfo* stepInfo, int type);
  
  void deleteStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback, StepInfo* stepInfo);
  
  tf2::Stamped<tf2::Transform> getExitPoseFromApproach(StepInfo const &stepInfo, tf2::Stamped<tf2::Transform> const &approachPose);
	
	double getCosts(CostSettings const&);
	
	inline static const CostSettings& selectCosts(const StepInfo& entry, EdgeStepData::StepType type) {
		return type == EdgeStepData::UP? entry.upCosts : (type == EdgeStepData::DOWN? entry.downCosts : (type == EdgeStepData::CROSS_OVER ?entry.crossOverCosts : entry.crossGravelCosts));
	}
};
