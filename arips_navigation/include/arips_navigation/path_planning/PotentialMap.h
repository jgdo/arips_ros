...
public:
    ...
    inline costmap_2d::Costmap2DROS& getCostmapRos() { return mCostmapRos; }

    inline const costmap_2d::Costmap2DROS& getCostmapRos() const { return mCostmapRos; }

    double goalDistance(int x, int y) const { return mMatrix(x, y).goalDist; }
...
private:
    ...
    std::unordered_map<Cell*, DistQueue::iterator>
        mLocationMap; /// location of a cell in the queue if any. TODO: consider using dense matrix
                      /// if too slow
