...
void FlatGroundModule::initNodeData(TopoMap::Node *node, CostsPlannerPtr planner,
                                    NodeMatrixPtr nodeMatrix, tf2_ros::Buffer *tfBuffer, int x,
                                    int y, std::string mapName, double height) {
    ...
    for (auto &mapEntry : getMapData(&map)) {
        ...
        if (std::abs(localPose.getOrigin().z()) <
            ...
            if (costmap_ros.getCostmap()->worldToMap(localPose.getOrigin().x(),
                ...
                if (realNode) {
                    localPose.getOrigin().setZ(
                        getNodeData(realNode)
                            .areaHeight); // lookup the configured height of this node
                    break;
                }
            }
        }
...
FlatGroundModule::computeCostsOnRegion(const TopoMap::Node *node, LocalPosition const &currentPos,
    ...
    tf2::Stamped<tf2::Transform> startTransformed =
        _context.tfBuffer->transform(srcData.pose, nodeData.planner->getMap().getGlobalFrameID());
    ...
    bool success =
        nodeData.planner->makePlan(startMsg, approach, plan, &distance, &actualApproachPose);
