...
void StepEdgeModule::beginParsing(TopoMap *map, YAML::Node const &parserConfig) {
  ...
  parser.parse(&currentStepData.defaultDownCosts, parserConfig["default_costs_down"], className, "1.0");
  parser.parse(&currentStepData.defaultUpCosts, parserConfig["default_costs_up"], className, "1");
  parser.parse(&currentStepData.defaultCrossOverCosts, parserConfig["default_costs_cross_over"], className, "1");
  parser.parse(&currentStepData.defaultCrossGravelCosts, parserConfig["default_costs_cross_gravel"], className, "1");
    ...
    std::string frame = YamlTools::parseFrameID(stepDef["frame_id"], "Error when parsing map step objects: frame_id for step '" + name + "' is empty");
    start.frame_id_ = frame;
    end.frame_id_ = frame;
    CostSettings upCosts = currentStepData.defaultUpCosts, downCosts = currentStepData.defaultDownCosts, crossCosts = currentStepData.defaultCrossOverCosts, crossGravelCosts = currentStepData.defaultCrossGravelCosts;
    parser.parse(&downCosts, stepDef["costs_down"], className, "stairs '" + name + "'down costs");
    parser.parse(&upCosts, stepDef["costs_up"], className, "stairs '" + name + "'up costs");
    parser.parse(&crossCosts, stepDef["costs_cross_over"], className, "stairs '" + name + "'cross over costs");
    parser.parse(&crossGravelCosts, stepDef["costs_cross_gravel"], className, "stairs '" + name + "'cross gravel costs");
    
		currentStepData.steps[name] = StepInfo{start, end, name, upCosts, downCosts, crossCosts, crossGravelCosts};
	}
...
void StepEdgeModule::parseEdgeData(YAML::Node const &config, TopoMap::Edge *edge) {
  ...
  if(type_str == "up")
    ...
    ROS_WARN_STREAM("step_type for step " << stepName << " has invalid value. Valid values are {'up', 'down', 'cross_over', 'cross_gravel'}. Using default value 'up'.");
	
	initEdgeData(edge, stepName, type);
	
	if(currentStepData.steps.find(stepName) == currentStepData.steps.end())
		ROS_WARN_STREAM("StepEdgeModule::parseEdgeData(): definition for step \"" << stepName << "\" could not be found");
}

std::pair<double, LocalPositionConstPtr> StepEdgeModule::computeTransitionCost(const TopoMap::Edge* edge, const LocalPosition &approachPose, boost::any *pathData) {
  ...
  return std::make_pair(costs, PositionDataPtr(new PositionData(getExitPoseFromApproach(entry, approach.pose))));
  
  /*
  if(ApproachLineAreaPtr approachLine = std::dynamic_pointer_cast<ApproachLineArea>(inData)) {
    tf2::Stamped<tf2::Transform> approach(tf2::Transform(approachLine->orientation, (approachLine->start + approachLine->end)*0.5), approachLine->orientation.stamp_, approachLine->orientation.frame_id_);
    ...
    ROS_WARN_STREAM("Failed to create exit position for edge '" << edge->getName() << "' since exit area type is unknown");
    return std::make_pair(costs, LocalPositionConstPtr()); // FIXME!!
  } */
}
...
void StepEdgeModule::visualizeEdge(const TopoMap::Edge *edge) {
  ...
  auto markerNames = createEdgeMarkers(edge, approachData, exitData, visualization_msgs::InteractiveMarkerControl::MENU);
    ...
    if(!menu_handler) {
      ...
      menu_handler->insert("Down", boost::bind(&StepEdgeModule::setEdgeUpDown, this, _1, edge));
      menu_handler->insert("Cross Over", boost::bind(&StepEdgeModule::setEdgeUpDown, this, _1, edge));
      menu_handler->insert("Cross Gravel", boost::bind(&StepEdgeModule::setEdgeUpDown, this, _1, edge));
      menu_handler->insert("Retrieve position from step", boost::bind(&StepEdgeModule::setApproachFromStepCB, this, _1, edge));
    }
    
    menu_handler->setCheckState(1, data.type == EdgeStepData::UP ? interactive_markers::MenuHandler::CHECKED : interactive_markers::MenuHandler::UNCHECKED);
    menu_handler->setCheckState(2, data.type == EdgeStepData::DOWN ? interactive_markers::MenuHandler::CHECKED : interactive_markers::MenuHandler::UNCHECKED);
    menu_handler->setCheckState(3, data.type == EdgeStepData::CROSS_OVER ? interactive_markers::MenuHandler::CHECKED : interactive_markers::MenuHandler::UNCHECKED);
    menu_handler->setCheckState(4, data.type == EdgeStepData::CROSS_GRAVEL ? interactive_markers::MenuHandler::CHECKED : interactive_markers::MenuHandler::UNCHECKED);
    
    for (auto const &name: markerNames) {
      menu_handler->apply(mapEditor->getMarkerServer(), name);
      //mapEditor->getMarkerServer().setCallback(name, boost::bind(&StepEdgeModule::setEdgeUpDown, this, _1, &data),
...
void StepEdgeModule::createNewMapStep(TopoMap *map, std::string name, tf2::Stamped<tf2::Vector3> const &start,
																			const tf2::Stamped<tf2::Vector3> &end) {
	MapStepData& stepData = getMapData(map);
	stepData.steps[name] = StepInfo{start, end, name, stepData.defaultUpCosts, stepData.defaultDownCosts, stepData.defaultCrossOverCosts, stepData.defaultCrossGravelCosts};
}

void StepEdgeModule::initEdgeData(TopoMap::Edge *edge, std::string stepName, EdgeStepData::StepType stepType) {
...
void StepEdgeModule::initializeVisualization(MapEditor *editor) {
  ...
  markerColor = tf2::Vector3(1, 0.5, 0);
	...
	mapEditor->getMenuHandler().insert(mapEditor->getCreateEdgesMenuHandle(), "Step Edge", boost::bind(&StepEdgeModule::createEdgeCB, this, _1));
	mapEditor->getMenuHandler().insert(mapEditor->getOtherMenuHandle(), "Create Map Step", boost::bind(&StepEdgeModule::createMapStepCB, this, _1));
}

void StepEdgeModule::selectMapStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback) {
...
void StepEdgeModule::deactivate() {
	vizState = IDLE;
}
...
void StepEdgeModule::poseCallback(geometry_msgs::PoseStamped const &msg) {
    ...
    tf2::Stamped<tf2::Vector3> end(newPose(tf2::Vector3(0, 0.5, 0.0)), newPose.stamp_, newPose.frame_id_);
    
		auto& stepsData = getMapData(mapEditor->getMap());
		StepEdgeModule::createNewMapStep(mapEditor->getMap(), findFreeStepName(stepsData), start, end);
  ...
  TopoMap::Node* node = mapEditor->getMap()->getNode(gp.node->getName());
    ...
    lastApproachPose = pose;
		...
		std::string nodeName = edgeType + "_" + std::to_string(mapEditor->getMap()->getNumEdges()) + "_" + lastApproachNode->getName() + "_" + node->getName();
		auto edge = mapEditor->getMap()->addEdge(lastApproachNode, node, nodeName, edgeType);
    setApproachData(edge, ApproachLineArea::create(lastApproachPose.getOrigin(), lastApproachPose.getOrigin(),
																																				 tf2::Stamped<tf2::Quaternion>(lastApproachPose.getRotation(), lastApproachPose.stamp_, lastApproachPose.frame_id_))); // FIXME usage of flat planner
		
		setExitData(edge, ApproachLineArea::create(pose.getOrigin(), pose.getOrigin(),
																																		 tf2::Stamped<tf2::Quaternion>(pose.getRotation(), pose.stamp_, pose.frame_id_)));
    
    // if frames are different, default direction is UP
    EdgeStepData::StepType stepType = (pose.frame_id_ == lastApproachPose.frame_id_ && pose.getOrigin().getZ() < lastApproachPose.getOrigin().getZ())?
                                      EdgeStepData::DOWN : EdgeStepData::UP;
...
void StepEdgeModule::createEdgeCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback) {
...
void StepEdgeModule::createMapStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback) {
...
YAML::Node StepEdgeModule::beginSaving(TopoMap *map) {
  ...
  YAML::Node steps(YAML::NodeType::Sequence);
    ...
    parser.storeDiff(step, "costs_cross_over", e.second.crossOverCosts, stepData.defaultCrossOverCosts);
    parser.storeDiff(step, "costs_cross_gravel", e.second.crossGravelCosts, stepData.defaultCrossGravelCosts);
...
void StepEdgeModule::setEdgeUpDown(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback,
...
static visualization_msgs::InteractiveMarker createEdgeTipMarker(tf2::Stamped<tf2::Vector3> pos, std::string name, std::string name_end) {
...
static visualization_msgs::InteractiveMarker createStepConnectionMarker(StepEdgeModule::StepInfo* step, std::string name) {
  visualization_msgs::InteractiveMarker int_marker;
...
void StepEdgeModule::visualizeMapStep(StepEdgeModule::StepInfo *stepInfo) {
  auto start = createEdgeTipMarker(stepInfo->start, stepInfo->name, "__start");
  ...
  mapEditor->getMarkerServer().insert(start, boost::bind(&StepEdgeModule::processMapStepCB, this, _1, stepInfo, 0));
  mapEditor->getMarkerServer().insert(end, boost::bind(&StepEdgeModule::processMapStepCB, this, _1, stepInfo, 1));
  
  auto stepMarker = createStepConnectionMarker(stepInfo, stepInfo->name+"_line");
  mapEditor->getMarkerServer().insert(stepMarker, boost::bind(&StepEdgeModule::processMapStepCB, this, _1, stepInfo, -1));
  
  if(!stepInfo->menuHandler) {
    stepInfo->menuHandler = std::make_shared<interactive_markers::MenuHandler>();
    stepInfo->menuHandler->insert("Delete this step", boost::bind(&StepEdgeModule::deleteStepCB, this, _1, stepInfo));
  }
  
  stepInfo->menuHandler->apply(mapEditor->getMarkerServer(), stepMarker.name);
}

void StepEdgeModule::processMapStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback,
...
void StepEdgeModule::setApproachFromStepCB(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback,
                                           const TopoMap::Edge *constEdge) {
...
void StepEdgeModule::parseEdgeInData(YAML::Node const &config, TopoMap::Edge *edge) {
  ...
  approach->getCenter();
  auto& data = getEdgeData(edge);
  auto exitPose = getExitPoseFromApproach(currentStepData.steps.at(data.stepName), approach->getCenter());
  setExitData(edge, std::make_shared<FixedPosition>(exitPose));
}

tf2::Stamped<tf2::Transform>
StepEdgeModule::getExitPoseFromApproach(StepInfo const &stepInfo, tf2::Stamped<tf2::Transform> const &approachPose) {
  tf2::Stamped<tf2::Transform> approachTransformed = _context.tfBuffer->transform(approachPose, stepInfo.start.frame_id_);
  
  // mirror the approach point at step line
  tf2::Vector3 normal = (stepInfo.end - stepInfo.start).normalized();
  tf2::Vector3 on_line = stepInfo.start + normal * (approachTransformed.getOrigin() - stepInfo.start).dot(normal);
...
double StepEdgeModule::getCosts(const BaseTraits<StepEdgeModule>::CostSettings &settings) {
  ...
  for(auto& varname: costs_comp_.getVariables()) {
    double cost;
    if(varname == CommonCostProfiles::TIME_COSTS || varname == CommonCostProfiles::DEFAULT_PROFILE) {
      ...
      cost = 1;
      ROS_WARN_STREAM(className << " error: unsupported cost profile variable '" << varname << "', using distance as costs.");
    }
