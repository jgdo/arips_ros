...
void FlatParser::beginParsing(TopoMap* map, YAML::Node const& parserConfig) {
    ...
    parser.parse(&costmapPlannerMap, parserConfig["default_costs"], FlatGroundModule::className,
                 "default costs");

    const YAML::Node mapsNode =
        parserConfig["layers"] ? parserConfig["layers"] : parserConfig["maps"];
      ...
      while(gridMapPlanner->getMap().getLayers().empty()) {
        ...
        if (plannerType == "NavfnCostsPlanner") {
            std::shared_ptr<costmap_2d::Costmap2DROS> costmap =
                std::make_shared<costmap_2d::Costmap2DROS>("costmaps/" + costmapName,
                                                           *_context.tfBuffer);
            costmap->start();

            std::shared_ptr<navfn::NavfnROS> navfn =
                std::make_shared<navfn::NavfnROS>(costmapName + "_planner", costmap.get());

            planner =
                std::make_shared<NavfnCostsPlanner>(costmap, navfn, costmapName, *_context.tfBuffer);
            ...
            ROS_ERROR_STREAM("Flat map level '"
                             << costmapName << "' specifies a UNKNOWN planner type '" << plannerType
                             << "'. Ingoring map level.");
            continue;
        }

        FlatGroundModule::NodeMatrixPtr nodeMatrix =
            std::make_shared<FlatGroundModule::NodeMatrix>(0, 0);
        const auto costmap = planner->getMap().getCostmap();
        *nodeMatrix = FlatGroundModule::NodeMatrix::Constant(costmap->getSizeInCellsX(),
                                                             costmap->getSizeInCellsY(), nullptr);
        costmapPlannerMap.emplace(costmapName, FlatGroundModule::MapPlanningDataEntry{
                                                   planner, nodeMatrix, _context.tfBuffer});
    }
}
...
void FlatParser::parseNodeData(YAML::Node const& config, TopoMap::Node* node) {
    ...
    std::string costmapName =
        (config["layer"] ? config["layer"] : config["map"]).as<std::string>(); // TODO make generic
    ...
    if (costmapIter == costmapPlannerMap.end()) {
        ROS_ERROR_STREAM("FlatParser: Could not find map layer entry '"
                         << costmapName << "' for node '" << node->getName() << "'. Available:");
        for (const auto& e : costmapPlannerMap) {
            ROS_ERROR_STREAM("\t" << e.first);
        }
    }

    FlatGroundModule::initNodeData(node, costmapIter->second.planner,
                                   costmapIter->second.nodeMatrix, _context.tfBuffer, nodeX, nodeY,
                                   costmapName, height);
    ...
    for (int i = 30; i > 0; i--) {
        ...
        const size_t cells =
            FlatGroundModule::regionGrow(node, costmapIter->second.planner->getMap().getCostmap(),
...
YAML::Node FlatParser::beginSaving(TopoMap* map) {
    ...
    for (auto& entry : data) {
      ...
      flat["planner"] = "GridMapCostsPlanner";
            ...
            ROS_ERROR_STREAM("Could not save flat map entry for map '"
                             << entry.first << "', since the planner type is unknown");
        }
